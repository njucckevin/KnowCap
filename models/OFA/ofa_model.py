...
class OFADecoder(OFAPreTrainedModel):
    ...
    def __init__(self, config: OFAConfig, embed_tokens: Optional[nn.Embedding] = None, output_projection=None):
        ...
        self.adapter = False
        if self.adapter == True:
            self.relu = nn.ReLU()
            self.adapter_ln1 = nn.Linear(self.output_embed_dim, self.output_embed_dim, bias=False)
            self.adapter_ln2 = nn.Linear(self.output_embed_dim, self.output_embed_dim, bias=False)
            nn.init.normal_(self.adapter_ln1.weight, mean=0, std=self.output_embed_dim ** -0.5)
            nn.init.normal_(self.adapter_ln2.weight, mean=0, std=self.output_embed_dim ** -0.5)
    ...
    def forward(
        ...
        if self.adapter == True:
            hidden_states = 0.5*hidden_states+0.5*self.adapter_ln2(self.relu(self.adapter_ln1(hidden_states)))
